                ...
                <div class="col-md-12">
                    ...
                    <div class="card mb-25">
                        <div class="card-body">
                            ...
                            @endif
                            <p class="fs-20 fw-bold">หมวดหมู่ที่ / Category No. {{ $category->cates_no }} :
                                {{ $category->chk_cats_name }}</p>
                            <form method="POST"
                                action="{{ route('guest.chk_insert_step2', [$record->record_id, $category->category_id]) }}"
                                    ...
                                    <div class="mb-3 border-success rounded p-3">
                                        ...
                                        @if ($item->item_type == '1')
                                            <select name="item_result[{{ $item->id }}]" class="form-select mt-2"
                                                ...
                                                <option value="2">⚠️ ไม่ผ่าน แต่ยังสามารถใช้งานได้/Abnormal but
                                                    functional</option>
                                        ...
                                        @elseif ($item->item_type == '2')
                                            <select name="item_result[{{ $item->id }}]" class="form-select mt-2"
                                                ...
                                                <option value="2">⚠️ ไม่ปกติ แต่ยังสามารถใช้งานได้ /Abnormal but
                                                    functional</option>
                                                <option value="0">❌ ไม่สามารถใช้งานได้/Not functional</option>
                                                <option value="3">⛔ ไม่เกี่ยวข้อง/Not applicable</option>
                                            </select>
                                        ...
                                        <textarea name="user_comment[{{ $item->id }}]" class="form-control mt-2"
                                            placeholder="ความคิดเห็นเพิ่มเติม (ถ้ามี)/ Comment (if any)"></textarea>

                                        <label class="mt-2">อัปโหลดภาพ (ไม่เกิน 5 ภาพ)/Upload images (maximum 5
                                            images)</label>
                                ...
                                <button type="submit" class="btn btn-block btn-success fs-18">บันทึกและไปต่อ / Save and
