@php
    ...
    $user = Auth::user();
    $role = $user->role; // role จริงใน DB (Enum)
    $activeRole = active_role(); // role ที่กำลังใช้งานจริงจาก session

    $agent = null;

    if ($user && $user->agency_id) {
        $agent = DB::table('users')->select('name')->where('id', $user->agency_id)->first();
    }
@endphp
    ...
    <ul class="sidebar_nav">
        ...
        <li class="menu-title mt-30">
            ...
            @elseif ($activeRole === 'user')
                ...
                <div class="border-top my-3"></div>
                <span><i class="fas fa-bars"></i> เมนูสำหรับเจ้าหน้าที่</span>

            @endif
        </li>

        {{-- เมนูฝั่ง Agency View --}}
        @if ($activeRole === 'agency')
